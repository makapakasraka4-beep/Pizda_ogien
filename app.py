...
TIME_FACTOR = 0.125 # 8 sekund statyki = 1 powtórzenie dynamiczne
...
try:
    ...
    st.error("Błąd: Nie skonfigurowano DB_URI w Secrets.")
    st.stop()
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS plan (id SERIAL PRIMARY KEY, kategoria TEXT, cwiczenie TEXT, opis TEXT, serie INTEGER, powtorzenia INTEGER, obciazenie REAL, pompa_rate INTEGER, username TEXT, masa_wlasna INTEGER DEFAULT 0, na_czas INTEGER DEFAULT 0, czas INTEGER DEFAULT 0)''')
    c.execute('''CREATE TABLE IF NOT EXISTS historia (id SERIAL PRIMARY KEY, data TEXT, kategoria TEXT, cwiczenie TEXT, serie INTEGER, powtorzenia INTEGER, obciazenie REAL, pompa_rate INTEGER, punkty_pompy REAL, username TEXT, masa_wlasna INTEGER DEFAULT 0, na_czas INTEGER DEFAULT 0, czas INTEGER DEFAULT 0)''')
...
def auth_screen():
    ...
    with col2:
        ...
        with tab1:
            u = st.text_input("Login", key="l_u"); p = st.text_input("Hasło", type="password", key="l_p")
            if st.button("Wejdź", type="primary", use_container_width=True):
                ...
                else: st.error("Zły login lub hasło")
        with tab2:
            ...
            if st.button("Zarejestruj", use_container_width=True):
                if nu and np:
                    ...
                    conn.commit(); st.success("Gotowe! Zaloguj się.")
    return False
...
def render_zarzadzanie_planem(kategoria_nazwa, ikona, kolor):
    ...
    for idx, row in df_plan.iterrows():
        ...
        obc = f"Masa wł. + {row['obciazenie']}kg" if row['masa_wlasna'] else f"{row['obciazenie']}kg"
        if row['na_czas']: c_a.write(f"**{row['cwiczenie']}** | {row['serie']}x {row['czas']}s | {obc}")
        else: c_a.write(f"**{row['cwiczenie']}** | {row['serie']}x{row['powtorzenia']} | {obc}")
        with c_b: 
    ...
    with st.form(f"f_{kategoria_nazwa}", clear_on_submit=True):
        ...
        powt = c3.number_input("Powt / Sek", min_value=1, value=int(d_cz) if d_nc else int(d_p))
        obc = c4.number_input("Kg", min_value=0.0, value=float(d_o))
        mw = c5.toggle("Masa wł.", value=bool(d_mw))
        if st.form_submit_button("ZAPISZ"):
            is_t = 1 if "czas" in typ.lower() else 0
            c = conn.cursor()
            if edit_id:
                c.execute("UPDATE plan SET cwiczenie=%s, serie=%s, powtorzenia=%s, obciazenie=%s, masa_wlasna=%s, na_czas=%s, czas=%s WHERE id=%s", 
                          (nazwa, serie, 0 if is_t else powt, obc, int(mw), is_t, powt if is_t else 0, edit_id))
            else:
                c.execute("INSERT INTO plan (kategoria, cwiczenie, serie, powtorzenia, obciazenie, username, masa_wlasna, na_czas, czas) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)", 
                          (kategoria_nazwa, nazwa, serie, 0 if is_t else powt, obc, usr, int(mw), is_t, powt if is_t else 0))
            conn.commit(); st.session_state[edit_key] = None; st.rerun()

def main():
    init_db(); 
    if not auth_screen(): return
    usr = st.session_state.username; rc = st.session_state.get('reset_counter', 0)
    conn = get_db_connection(); cats = pd.read_sql_query("SELECT * FROM categories WHERE username=%s OR username='Główny'", conn, params=(usr,))
    
    t_names = ["📋 Trening", "📈 Staty", "📏 Waga"] + [f"{r['icon']} {r['name']}" for _, r in cats.iterrows() if r['username'] != 'Główny' or (r['name'] not in cats[cats['username']==usr]['name'].values)] + ["⚙️ Setup"]
    all_tabs = st.tabs(t_names)

    with all_tabs[0]:
        wybrany = st.radio("Trening:", cats['name'].tolist(), horizontal=True)
        cat_info = cats[cats['name'] == wybrany].iloc[0]
        st.markdown(f"<h3 style='color: {cat_info['color']};'>{cat_info['icon']} {wybrany}</h3>", unsafe_allow_html=True)
        plan = pd.read_sql_query("SELECT * FROM plan WHERE kategoria=%s AND username=%s", conn, params=(wybrany, usr))
        if not plan.empty:
            ...
            for i, r in plan.iterrows():
                ...
                s = c2.number_input("Ser", value=int(r['serie']), key=f"s_{r['id']}_{rc}")
                if r['na_czas']: 
                    cz = c3.number_input("Sek", value=int(r['czas']), key=f"cz_{r['id']}_{rc}"); p = 0
                else: 
                    p = c3.number_input("Pow", value=int(r['powtorzenia']), key=f"p_{r['id']}_{rc}"); cz = 0
                ...
                pr = c6.number_input("Pompa", 1, 5, 3, key=f"pr_{r['id']}_{rc}")
            ...
            if st.button("ZAPISZ", type="primary", use_container_width=True) and zrobione:
                ...
                for z in zrobione:
                    ...
                    # MATEMATYKA Z PRZELICZNIKIEM CZASU
                    rep_equiv = (z['cz'] * TIME_FACTOR) if z['nc'] else z['p']
    ...
    with all_tabs[1]:
        ...
        if not hist.empty:
            ...
            for d in hist['data'].unique():
                df_day = hist[hist['data']==d]
                with st.expander(f"🗓️ {d} | 🔥 {df_day['punkty_pompy'].sum():.0f} pkt"):
                    for _, r in df_day.iterrows():
                        c_h1, c_h2 = st.columns([4, 1])
                        c_h1.write(f"🗑️ {r['cwiczenie']} - {r['punkty_pompy']:.0f} pkt")
                        if c_h2.button("Usuń", key=f"dh_{r['id']}"):
    ...
    with all_tabs[2]:
        ...
        p_df = pd.read_sql_query("SELECT * FROM pomiary WHERE username=%s", conn, params=(usr,))
        if not p_df.empty:
            p_df['data'] = pd.to_datetime(p_df['data']); p_df = p_df.sort_values('data')
            st.plotly_chart(px.line(p_df, x='data', y='waga', markers=True), use_container_width=True)
    ...
    with all_tabs[-1]:
        with st.form("cat"):
            n = st.text_input("Nazwa"); i = st.selectbox("Ikona", ["🏋️", "💪", "🔥", "🏃", "🤸"]); col = st.color_picker("Kolor", "#FFD700")
            if st.form_submit_button("DODAJ"):
                c = conn.cursor(); c.execute("INSERT INTO categories (name, username, icon, color) VALUES (%s,%s,%s,%s)", (n, usr, i, col)); conn.commit(); st.rerun()
        for _, r in cats[cats['username']==usr].iterrows():
